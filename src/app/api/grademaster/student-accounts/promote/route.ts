...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: behaviorsToPromote, error: behaviorSelectError } = await supabase
      .from('gm_behaviors')
      .select('student_name, total_points, behavior_logs')
    ...
    // 3. Masukkan baris baru di gm_behaviors untuk pencatatan perilaku di kelas & tahun ajaran baru dengan mempertahankan poin & logs
    const behaviorRows = behaviorsToPromote.map(b => ({
      student_name: b.student_name,
      class_name: toClass.trim(),
      academic_year: toYear.trim(),
      total_points: b.total_points ?? 0,
      behavior_logs: b.behavior_logs ?? []
    }));
