...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-3 bg-surface-container/80 backdrop-blur-xl rounded-t-[1.5rem] shadow-2xl shadow-black/20">
      {tabs.map((tab) => {
        ...
        return (
          <Link
            ...
            className={`
              ...
              ${isActive ? 'bg-surface-bright text-on-surface rounded-[1rem]' : 'text-on-surface-variant hover:text-on-surface'}
            `}
          >
