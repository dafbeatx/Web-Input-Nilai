...
export default function StudentProfileLayer({ 
  ...
  const [startingRemedialSessionName, setStartingRemedialSessionName] = useState<string | null>(null);
  ...
  const handleStartRemedial = async (sessionName: string) => {
    if (!onStartRemedial) return;
    setStartingRemedialSessionName(sessionName);
    try {
      await Promise.resolve(onStartRemedial(sessionName));
    } catch (err: any) {
      setToast({ message: err.message || 'Gagal memulai sesi remedial', type: 'error' });
    } finally {
      setStartingRemedialSessionName(null);
    }
  };
        ...
        {activeTab !== 'LESSON' ? (
            ...
            <div className="space-y-5 animate-in fade-in duration-300 text-left">
              ...
              {!isAdmin && !isParent && (
                <div className="p-5 bg-gradient-to-br from-indigo-50/80 to-purple-50/80 border border-indigo-100 rounded-3xl text-left relative overflow-hidden shadow-sm animate-in slide-in-from-bottom-2 duration-300">
                  ...
                  <button
                    onClick={async () => {
                      // Kirim notifikasi tracking ke Telegram
                      try {
                        const deviceInfo = typeof window !== 'undefined' ? navigator.userAgent : 'unknown';
                        await fetch('/api/telegram/notify', {
                          ...
                          body: JSON.stringify({
                            event: 'PHOTO_CLAIM',
                            message: `📸 <b>KLAIM FOTO SISWA</b>\n\n👤 Siswa: <b>${studentName}</b>\n🏫 Kelas: <b>${activeClass}</b>\n📅 Tahun Ajaran: <b>${activeYear}</b>\n📱 Perangkat: <code>${deviceInfo}</code>\n\nSiswa telah mengklik tombol klaim foto dan diarahkan ke Google Drive.`,
                          }),
                        });
                      } catch (err) {
                        console.error('Failed to send Telegram tracking:', err);
                      }
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar pt-safe">
            <StudentLessonLayer
              ...
              semester={semester}
              isTab={true}
              studentClassOverride={className}
            />
          </div>
        )}
              ...
              {(() => {
                ...
                if (totalPoints === 0) {
                  if (isGood) {
                    ...
                    return (
                      <div className="py-12 text-center flex flex-col items-center justify-center border border-dashed rounded-2xl bg-emerald-50/20 border-emerald-100">
