      ...
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white rounded-2xl md:rounded-[2.5rem] p-4 sm:p-6 md:p-8 shadow-xl shadow-slate-200/50 border border-slate-100 space-y-4 md:space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            ...
            {/* Kunci Jawaban Essay Remedial */}
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <label className={labelClass}><Key size={12} className="md:w-3.5 md:h-3.5" /> Kunci Jawaban Essay Remedial</label>
              ...
              <textarea
                ...
                placeholder={"Ketikkan kunci jawaban essay di sini.\nFormat sama seperti soal:\n1. Internet adalah jaringan komputer global...\n2. Next.js adalah framework React...\n3. Supabase adalah platform backend..."}
                rows={6}
                className={`${inputClass} resize-y font-normal`}
              />
              {remedialAnswerKeys.length > 0 && (
                <div className="p-3 bg-emerald-50 rounded-xl border border-dashed border-emerald-200 mt-2">
                  <p className="text-[9px] font-black uppercase tracking-widest text-emerald-500 mb-2">Preview Kunci Jawaban</p>
                  <div className="space-y-2">
                    {remedialAnswerKeys.map((k, idx) => (
                      <div key={idx} className="flex gap-2 text-[10px] md:text-xs">
                        <span className="font-black text-emerald-600">{idx + 1}.</span>
                        <span className="text-slate-600 font-medium line-clamp-1">{k}</span>
                      </div>
                    ))}
                  </div>
                </div>
